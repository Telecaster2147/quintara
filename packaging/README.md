...
- `packaging/developer_data/quintara-developer-data-v1.zip`：随安装目录交付的开发者数据，安装后位于 `data/developer/`，原始包不写入 AppData。
...
安装包不包含用户账户、token 或既有训练历史。开发者数据包与应用放在同一安装目录树；首次向导校验后在用户选择的工作目录建立活动 generation。
