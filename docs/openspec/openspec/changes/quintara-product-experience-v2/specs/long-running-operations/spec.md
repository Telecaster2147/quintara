...
### Requirement: 数据更新阶段支持可验证续作
**LRO-006** BaoStock 初始化与更新 SHALL 将请求身份、阶段、已完成证券和缓存文件原子记录；继续执行前 SHALL 核对连接器、代码集合、日期范围、字段、复权和股票池路线，身份变化时重新规划而不复用旧分块。

#### Scenario: 进程在行情阶段退出
- **WHEN** 用户重新启动并再次确认相同请求
- **THEN** 应用只复用身份和哈希匹配的已完成证券分块，并继续显示真实完成数量

#### Scenario: BaoStock 更新实时可观察
- **WHEN** 用户确认 BaoStock 更新计划且任务经过连接、证券资料、股票池、行情、校验和发布阶段
- **THEN** 数据页持续显示按时间排序的自然语言阶段日志、真实进度和已用时间，并在完成、取消或失败时保留最后阶段与可执行后续操作

### Requirement: 训练实时日志与异常终态
**LRO-007** Quintara SHALL 在训练开始后实时展示用户友好的检查、数据准备、模型训练、排名、组合分析、发布与完成阶段，包含阶段进度和已用时间；任务失败、协作停止、进程异常退出或训练组件加载失败时 SHALL 记录最后阶段、错误类型、脱敏技术信息、对已有结果的影响和可执行恢复动作，而不是只显示“训练失败”。

#### Scenario: 完整 CPU 训练
- **WHEN** 用户从训练页启动一次超过一秒的 CPU 任务
- **THEN** 页面持续显示按时间排序的阶段日志、进度和已用时间，成功后自动进入已发布 Top-5 结果

#### Scenario: 训练抛出异常
- **WHEN** 数据标签、LightGBM 动态库、内存、磁盘或其他训练步骤抛出异常
- **THEN** 页面保留已完成阶段，新增明确失败日志和恢复建议，技术详情记录异常类型与消息，应用进程继续可用
