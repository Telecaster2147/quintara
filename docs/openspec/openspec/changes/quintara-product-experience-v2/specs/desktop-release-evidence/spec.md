...
**DRE-002** 发布流水线 SHALL 在隔离用户目录中通过鼠标/键盘或 Qt 语义接口完成首次声明、安装包/BaoStock/CSV 三类数据来源的初始化与增量更新、数据检查、股票池、训练、Top-5、历史和 CSV 导出，并断言用户可见文本、状态与产物内容；至少一条 Ubuntu 本机旅程 SHALL 使用随包完整开发者数据并验证输出与参考 CSV 字节级一致，Windows Actions SHALL 执行相同三来源 GUI 合同旅程。
...
**DRE-004** Windows 安装器、Windows 便携包和 Linux 发行物 SHALL 验证首次启动、快捷方式或入口、QML/Qt 资源、内置开发者数据、联网数据下载、本地数据目录、升级保留、卸载边界和 CLI；许可证及法律材料 SHALL 随发行物可访问。三类发行物 SHALL 在应用目录树的 `data/developer/` 包含同一数据包，且单独的无 sidecar 便携 EXE 不作为发布资产。
...
### Requirement: 发行物训练与完整性实测
**DRE-007** 完整安装包 SHALL 显式包含 LightGBM 原生运行库、Qt/QML 插件和版本化开发者数据；发布前 SHALL 在隔离本地根执行安装包归档完整性检查、文件解包核对及至少一次“导入开发者数据—CPU 训练—Top-5—参考哈希”旅程，并记录安装器哈希、文件大小、训练阶段日志和结果哈希。

#### Scenario: 本地完整安装包候选
- **WHEN** 最新 Windows 安装包完成构建
- **THEN** 安装器自校验与独立归档测试通过，开发者数据哈希与源包一致，打包后的训练依赖可发现，并完成一次可追溯训练结果核对
