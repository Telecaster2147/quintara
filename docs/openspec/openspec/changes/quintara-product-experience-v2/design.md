...
- 以版本化、可校验、可原子回滚的数据包交付比赛生产数据，并让 BaoStock 增量更新接续该数据。
...
- `competition-open-open-v1`：原比赛的 open/open 差分夹具，仅用于内核回归与生产数据可重复性检查。
- `quintara-close5-open1-v1`：产品默认 `close(T+5) / open(T+1) - 1`，交易日历驱动，用于面向用户的训练和结果。

任何 dataset/model/result manifest 都写入标签合同，适配器在训练前 fail-closed 校验。供应方数据可以源于比赛生产管线，但面向用户训练仍按所选产品标签重新构造目标，二者概念分离。
