...
| PPD-008–011 | `data_lifecycle.py`, `service.py`, `QmlBackend`, BaoStock preview dialog | `test_baostock_one_click.py` 三来源派生、PIT 变化/退市保留、远端交易日、OHLC 失败原子性；最小窗口预览对话框与实时更新日志 QML shell tests |
...
| DPE-009 | cached result analytics, deferred QML actions, workspace and preview-dialog ScrollViews | page-latency regression、minimum-window scrollbar/confirmation reachability、QML structural checks |
...
| LRO-006 | BaoStock request identity/checkpoint、GUI stage signals and data-page live logs | checkpoint identity assertions、cancel/failure publication tests、data-workspace live-stage regression |
| LRO-007 | `QuintaraService.run(progress=...)`, QML live training panel and friendly terminal errors | service phase callback、backend success/failure logs、developer-data GUI training journey |
...
| DRE-007 | explicit LightGBM native-library collection and complete installer verification | PyInstaller archive audit、installer archive test/extraction、developer-data hash、packaged CPU training evidence |

反向检查：`tests/test_contracts_v2.py` 对应 FRC/PPD/RRW 身份闭包；`tests/test_v2_product_infrastructure.py` 对应 PPD/LRO/DRE；`tests/test_qml_product_shell.py` 对应 FRC/DPE/DRE；`packaging/linux_user_journey.py` 与 `packaging/developer_data_journey.py` 对应 DRE-002/004/005/007；`packaging/rollback_drill.py` 对应 catalog 回滚。`packaging/test_matrix.py` 汇总单元、属性、合同、GUI、CLI、恢复和本地视觉检查，`packaging/openspec_audit.py` 对全部 45 个需求 ID 做双向覆盖检查。正式 Linux release 必须在 Ubuntu 22.04/glibc 2.35 builder 通过 ABI 审计；原生平台证据缺少时 DRE-001/004/005 保持候选门禁，offscreen 结果只作为快速预检；`candidate_gate.py` 和 `dist/release-evidence.json` 将这些状态明确标为 `pre-release`。
